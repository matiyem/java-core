...
//    مرجع اتمی باید در تنظیماتی استفاده شود که در آن شما نیاز به انجام عملیات اتمی ساده (مثلاً بدون مشکل، غیر ضروری) روی یک مرجع دارید، که همگام سازی مبتنی بر مانیتور برای آن مناسب نیست. فرض کنید می‌خواهید بررسی کنید که آیا یک فیلد خاص فقط در صورتی است که وضعیت شی همان‌طور که آخرین بررسی کرده‌اید باقی بماند:
//    به دلیل معناشناسی مرجع اتمی، می‌توانید این کار را انجام دهید، حتی اگر شی حافظه پنهان بین رشته‌ها به اشتراک گذاشته شده باشد، بدون استفاده از همگام‌سازی. به طور کلی، بهتر است از همگام‌کننده‌ها یا چارچوب java.util.concurrent به جای Atomic* برهنه استفاده کنید، مگر اینکه بدانید در حال انجام چه کاری هستید.
...
//        جاوا کلاس Pair<T, U> عمومی در API عمومی خود ندارد. دلیل اصلی این امر این است که ممکن است به جای ایجاد انواع متمایز، وسوسه شویم که از آن بیش از حد استفاده کنیم.
        boolean[] markHolder = new boolean[1];//مقدار دیفالت برابر با false است
